...
from plotting.plot_variants import \
    all_plot_variants as plot_variants
from plotting.utils import (compute_all_metrics,
...
def load_all_results():
    ...
    for mode in ["non-batch", "batch"]:
        for properties, f in ann_results.load_all_results(batch_mode=(mode == "batch")):
            sdn = get_run_desc(properties)
            if sdn != old_sdn:
                dataset, _ = ann_datasets.get_dataset(properties["dataset"])
