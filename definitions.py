...
def get_config_files(base_dir: str = "algorithms") -> List[str]:
...
def load_configs(point_type: str, base_dir: str = "algorithms") -> Dict[str, Any]:
...
def _get_definitions(base_dir: str = "algorithms") -> Dict[str, Dict[str, Any]]:
    """Load algorithm configurations for a given point_type."""
    config_files = get_config_files(base_dir=base_dir)
    configs = {}
    for config_file in config_files:
...
def _get_algorithm_definitions(point_type: str, distance_metric: str, base_dir: str = "algorithms") -> Dict[str, Dict[str, Any]]:
    ...
    metric. For example, `algorithms.nmslib` has two definitions for euclidean float
    ...
    for _, config in configs.items():
        ...
        for cc in c:
            # 忽略 disabled=true 的算法
            if not cc.get("disabled", False):
                definitions[cc.pop("name")] = cc

    return definitions

def list_algorithms(base_dir: str = "algorithms") -> None:
    """
...
def create_definitions_from_algorithm(name: str, algo: Dict[str, Any], dimension: int, distance_metric: str = "euclidean", count: int = 10) -> List[Definition]:
    """Create definitions from an individual algorithm."""
    required_properties = ["module", "constructor"]
