...
from ann_datasets import get_dataset
from plotting.metrics import all_metrics as metrics
from plotting.utils import (compute_metrics, create_linestyles,
                                           create_pointset, get_plot_label)
from ann_results import get_unique_algorithms, load_all_results


def create_plot(all_data, raw, x_scale, y_scale, xn, yn, fn_out, linestyles, x_min=None):
    ...
    if "lim" in xm and x_scale != "logit":
        x0, x1 = xm["lim"]
        # X轴最小值
        x_min_value = x_min if x_min is not None else min_x
        plt.xlim(x_min_value, min(x1, 1))
    elif x_scale == "logit":
        x_min_value = x_min if x_min is not None else min_x
        plt.xlim(x_min_value, max_x)
    elif x_min is not None:
        # 如果设置了x_min但没有其他限制，则只设置最小值
        plt.xlim(left=x_min)
    if "lim" in ym:
        plt.ylim(ym["lim"])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--x-min",
        type=float,
        help="Minimum value for X-axis. If not set, auto-determined from data",
        default=None
    )
    parser.add_argument(
    ...
    create_plot(
        runs, args.raw, args.x_scale, args.y_scale, args.x_axis, args.y_axis, args.output, linestyles, args.x_min
    )
