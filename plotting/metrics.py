...
def get_recall_values(dataset_distances, run_distances, count, threshold, epsilon=1e-3):
    ...
    def process_item(args):
        i, item = args
        t = threshold(dataset_distances[item[1][0]], count, epsilon)
        actual = 0
        for d in item[0][:count]:
            if d <= t:
                actual += 1
        return actual
    ...
    # 确定使用的线程数，默认使用可用核心数的2倍，因为线程开销较小
    num_workers = max(1, multiprocessing.cpu_count() - 2)
    
    # 使用ThreadPoolExecutor进行并行处理
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        # 创建任务列表
        tasks = [(i, item) for i, item in enumerate(run_distances)]
        # 并行执行任务
...
def index_size(queries, attrs):
    ...
    return attrs.get("index_size", 0) / 1024 / 1024

def build_time(queries, attrs):
    return attrs.get("build_time", 0)

def create_index_time(queries, attrs):
    return attrs.get("create_index_time", 0)

def copy_time(queries, attrs):
    return attrs.get("copy_time", 0)

def table_size(queries, attrs):
    return attrs.get("table_size", 0) / 1024 / 1024
...
all_metrics = {
    ...
    "index_time": {
        "description": "Create index time (s)",
        "function": lambda true_distances, run_distances, metrics, times, run_attrs: create_index_time(
            true_distances, run_attrs
        ),  # noqa
        "worst": float("inf"),
    },
    "copy_time": {
        "description": "Copy time (s)",
        "function": lambda true_distances, run_distances, metrics, times, run_attrs: copy_time(
            true_distances, run_attrs
        ),  # noqa
        "worst": float("inf"),
    },
    "table_size": {
        "description": "Table size (MB)",
        "function": lambda true_distances, run_distances, metrics, times, run_attrs: table_size(
            true_distances, run_attrs
        ),  # noqa
        "worst": float("inf"),
    },
    ...
    "queriessize": {
        "description": "Index size (MB)/Queries per second (s)",
        "function": lambda true_distances, run_distances, metrics, times, run_attrs: index_size(
            true_distances, run_attrs
        )
